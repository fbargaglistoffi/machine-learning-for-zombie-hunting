...
prova = c("control", "shareholders_funds", "nace",
                "added_value", "cash_flow", "ebitda",
                "consdummy", "capital_intensity", "fin_cons100",
                "inv", "ICR_failure", "interest_diff", "NEG_VA")
...
```{r warning = FALSE}
time <- list()
#c("SL.glm", "SL.rpartPrune", "SL.randomForest", "SL.xgboost", "SL.bartMachine")
...
for (i in 1:n.folds){
      ...
      SL_cv <- SuperLearner(Y = Y, 
                            X = X, 
                            newX = M,
                            SL.library = c("SL.bartMachine.better") , 
                            verbose = FALSE, 
                            method = "method.NNLS", 
                            family = binomial(link='logit'), 
                            cvControl = list(V = 2) )
      
  #  prova = SL.bartMachine.better(Y = y, X = X, newX = M)

...
```{r}
time = list()
thresholds = seq.default(from = 0.5, to = 0.95, by = 0.05)
...
write.csv(xgboost_thresh_fit, file = "xgboost_thresh_fit.csv")
```
...
## Missing values imputation with unconditional mean imputation
```{r}
data_missing = data_italy[c("failure", predictors)]

# Imputation with CART
temp = mice(data_missing, method = "mean", m = 1, remove.collinear = FALSE)

# Get imputed data
data_imputed_mean = complete(temp)
```
BART - without MIA - on imputed data with mean imputation
```{r}
time <- list()
for (i in 1:n.folds){
      ...
      # Segment the data  
      index <- folds[[i]]
      test <- data_imputed_mean[index, ]
      train <- data_imputed_mean[-index, ]
...
write.csv(bart_fit, file = "bart_mean_imputation_fit.csv")
```
...
# Imputation with KNN
# temp = knnImputation(data_missing, k = 3, scale = TRUE, meth = "weighAvg")

# Imputation with CART
temp = mice(data_missing, method = "cart", m = 1)
...
write.csv(bart_fit, file = "bart_cart_fit.csv")
```

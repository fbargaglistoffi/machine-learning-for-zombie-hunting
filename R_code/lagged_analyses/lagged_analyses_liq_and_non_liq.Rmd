...
This \texttt{R Markdown} file reproduces the pooled machine learning analysis INCLUDING liquidations for the paper \textit{"Machine learning for zombie hunting. Firms' failures, financial constraints, and misallocation"} by Falco J. Bargagli-Stoffi (Harvard University), Fabio Incerti (IMT School for Advanced Studies of Lucca), Massimo Riccaboni (IMT School for Advanced Studies) and Armando Rungi (IMT School for Advanced Studies). 


## Packages Upload

The following packages and functions are the ones used for the analyses performed in the \texttt{R} code. The \texttt{functions.R} file contains the functions \texttt{F1\_score}, \texttt{balanced\_accuracy}, \texttt{model\_compare} and \texttt{DtD} and others that were developed to reproduce the following analyses. 
...
Firms in liquidation (7221): do not remove it. For the analyses without liquidations uncomment the following chunk
```{r}
...
## International descriptives 

Number of firms per country, failures and shares
```{r eval = FALSE}
table(data$iso)
...
Missing crucial variables across countries 
```{r eval = FALSE}
...
paste("Number of sample failures (liquidations included): ", sum(data$failure))
...
write.csv(logit_fit, file = "logit_complete_case__liq_fit.csv")
```
...
write.csv(ctree_fit, file = "ctree_complete_case_liq_fit.csv")
```
...
write.csv(rf_fit, file = "rf_complete_case_liq_fit.csv")
```
...
write.csv(xgboost_fit, file = "xgboost__complete_case_liq_fit.csv")
```
...
write.csv(bart_fit, file = "bart_complete_case_liq_fit.csv")
```
...
write.csv(sl_fit, file = "sl_complete_case_liq_fit.csv")
write.csv(tot_coef, file = "sl_complete_case_liq_coef.csv")
```
...
write.csv(bart_missing_fit, file = "bart_cc_missing__liq_fit.csv")
```
...
write.csv(xgboost_fit, file = "xgboost_cc_missing_liq_fit.csv")
```
...
write.csv(logit_fit, file = "logit_liq_fit.csv")
```
...
write.csv(ctree_fit, file = "ctree_liq_fit.csv")
```
...
write.csv(rf_fit, file = "rf_liq_fit.csv")
```
...
for (i in 1:n.folds){
    ...
    set.seed(3052)
    tic()
    xgboost_cv <- xgboost(data = train[, 2:ncol(train)], label = train[,1], objective = "binary:logistic", nrounds = 100)
    time[[i]]  <- toc()
...
write.csv(xgboost_fit, file = "xgboost_liq_fit.csv")
```
...
```{r}
time = list()
thresholds = seq.default(from = 0.5, to = 1, by = 0.02)
...
write.csv(xgboost_fit, file = "xgboost_fine_tuned_liq_fit.csv")
write.csv(mytune$x, file = "xgboost_tuned_liq_par.csv")
```
...
write.csv(bart_fit, file = "bart_liq_fit.csv")
```
...
write.csv(sl_fit, file = "sl_liq_fit.csv")
write.csv(tot_coef, file = "sl_liq_coef.csv")
...
write.csv(logit_fit, file = "logit_median_imputation_liq_fit.csv")
```
...
write.csv(ctree_fit, file = "ctree_median_imputation_liq_fit.csv")
```
...
write.csv(rf_fit, file = "rf_median_imputation_liq_fit.csv")
```
...
write.csv(xgboost_fit, file = "xgboost_median_imputation_liq_fit.csv")
```
...
write.csv(bart_fit, file = "bart_median_imputation_liq_fit.csv")
```
...
write.csv(sl_fit, file = "sl_median_imputation_liq_fit.csv")
write.csv(tot_coef, file = "sl_median_imputation_liq_coef.csv")

...
\par The "robustness" of the predictions is tested with respect to the inclusion of new predictors uncorrelated with varying correlations with the outcome (from 0.1 to 0.9).

\par In particular, this check is performed by generating a new predictor, a "confounder" $R_i$, orthogonal to the set of predictors $\mathbf{X}$ and with a correlation with the outcome that varies between 0.1 and 0.5, and checking how the inclusion of this additional predictor affects on the original model.
...
y_train <- as.factor(train_bart$failure)
```
...
axis(1, at=1:(length(sensitivity$`RMSE synthetic model`)), labels=c(seq(0.1,0.5,0.1)))
```
...
axis(1, at=1:(length(sensitivity$`Rsquared synthetic model`)), labels=c(seq(0.1,0.5,0.1)))
```
...
use data_clean_2805.dta

*Re-naming

rename P_L_for_period___Net_income__EUR  net_income_2016
rename P_L_for_period___Net_income__EU0 net_income_2015
rename P_L_for_period___Net_income__EU1 net_income_2014
rename P_L_for_period___Net_income__EU2 net_income_2013
rename P_L_for_period___Net_income__EU3 net_income_2012
rename P_L_for_period___Net_income__EU4 net_income_2011
rename P_L_for_period___Net_income__EU5 net_income_2010
rename P_L_for_period___Net_income__EU6 net_income_2009
rename P_L_for_period___Net_income__EU7 net_income_2008

*Cleaing the data
drop if iso=="GB" | iso=="PL"

count if id==""
count if Date_of_incorporation=="" /* 58,442 without data of incorporation*/
...
/*30% of failed has no status date*/

tab Status if failure==1 & Status_date==""
tab Status if Status!="Active" & Status!="Inactive (no precision)" & Status_date==""
tab iso  if failure==1 & Status_date==""
...
count if failure==1 & year_of_status==. /* again 12,640 without failure date if failed*/
count if year_of_incorporation==. /*again 58,442 with no year of incorporation*/
...
replace year_of_status=. if failure==0 /* CHECK THIS! */
...
*edit if  year_of_status < year_of_incorporation & year_of_incorporation!=. 

/*Bias in the record: status date instead of date of incorporation */
count  if  year_of_status!=. & failure==0
tab year_of_status if failure==0
tab iso if year_of_status!=. & failure==0
tab Status if year_of_status!=. & failure==0
*edit if year_of_status!=. if failure==0
...
replace year_of_status=2018 if failure==0 /* censure date */

drop if failure==1 & year_of_status==. 
tab year_of_incorporation if failure==1 & year_of_status==. 
tab iso if failure==1 & year_of_status==. 
tab failure iso
*edit if failure==1 & year_of_status==. 
drop if year_of_incorporation==. /*just 46 */
drop if failure==1 & year_of_status==. 

tab failure iso
drop if iso=="DE" | iso=="RO" | iso=="SE"
...
/* inactive firms are just in italy */
...
/*In the italian data many imputed values of revenues (all the same) */
...
/* Come trattare revenue imputate? */

*edit if revenue2016==revenue2015 & revenue2016!=.
*drop if revenue2016==revenue2015 & revenue2016!=. & revenue2016!=0 & iso=="IT" & revenue2012==. & revenue2011==.
*edit if revenue2015==revenue2014 & revenue2015!=. &  revenue2015!=0 & iso=="IT" & revenue2012==.
*drop if revenue2015==revenue2014 & revenue2015!=. &  revenue2015!=0 & iso=="IT" & revenue2012==. & revenue2011==.
*edit if revenue2014==revenue2013 & revenue2014!=. &  revenue2014!=0 & iso=="IT" & revenue2012==. & revenue2011==.
*drop if revenue2014==revenue2013 & revenue2014!=. &  revenue2014!=0 & iso=="IT" & revenue2012==. & revenue2011==.

tab year_of_status iso

tab year_of_incorporation iso
...
save data_lagged_14_07.dta, replace
restore
